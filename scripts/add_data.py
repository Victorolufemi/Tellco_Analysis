...
#    df = preprocess_df(df)
    ...
    for _, row in df.iterrows():
        sqlQuery = f"""INSERT INTO {table_name} (user_id, Experience_score, Engagement_score, Satisfactory_score)
             VALUES(%s, %s, %s, %s);"""
...
if __name__ == "__main__":
    createDB(dbName='scores')
    emojiDB(dbName='scores')
    createTables(dbName='scores')

    df = pd.read_csv('../notebook/final_table.csv')

    insert_to_tweet_table(dbName='scores', df=df, table_name='final_scores')
